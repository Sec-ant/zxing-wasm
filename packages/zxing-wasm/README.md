...
Run these commands from the repository root. The source package lives in `packages/zxing-wasm`; the root workspace scripts coordinate native compilation, package builds, and documentation.
...
`pnpm build` builds the npm package and generated API docs. Use `pnpm build:all` to initialize `vendor/zxing-cpp`, compile WASM, and then run the complete package build.
...
A local Vite scanning workbench is included in the `apps/demo` workspace. It supports multi-file and folder input, drag-and-drop, clipboard paste, per-file results, and the complete reader-option surface while consuming the library through its public workspace package dependency:
...
```ts
prepareZXingModule({
  overrides: {/* ... your desired overrides ... */},
}); // <-- returns void
```
...
- [packages/zxing-wasm/src/cpp/ZXingWasm.cpp](https://github.com/Sec-ant/zxing-wasm/blob/main/packages/zxing-wasm/src/cpp/ZXingWasm.cpp): [Apache License 2.0](https://github.com/Sec-ant/zxing-wasm/blob/main/packages/zxing-wasm/src/cpp/LICENSE)
